...
namespace rental.Controllers
{
    ...
    {
        // In-memory rented equipment list
        private static readonly List<RentViewModel> _rentedEquipmentList = new List<RentViewModel>();
        private static readonly List<Equipment> _equipmentList = new List<Equipment>
    {
        ...
        [HttpPost]
        public IActionResult Index(RentViewModel model)
        {
            if (ModelState.IsValid)
            {
                // Simulate saving rented equipment to in-memory list
                _rentedEquipmentList.Add(model);

                // Set success message
                ViewBag.SuccessMessage = "Equipment rented successfully!";


                // Or return the view with success message displayed
                return View(model);
            }

            // If model is not valid, return view with errors
            return View(model);
        }

        [HttpGet]
        public IActionResult RentForm(int equipment_id)
        {
            var equipment = _equipmentList.FirstOrDefault(e => e.Id == equipment_id);
            if (equipment == null)
            {
                return NotFound(); // Handle case where equipment with given ID is not found
            }
            ...
            return View("Index", viewModel); // Render Rent/Index.cshtml view with viewModel
        }
    }
}
